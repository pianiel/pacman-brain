    ...
    for i in range(len(self.agents)):
      ...
      if ("registerInitialState" in dir(agent)):
        ...
        if self.catchExceptions:
          timed_func = TimeoutFunction(agent.registerInitialState, int(self.rules.getMaxTotalTime(i)))
          try:
    ...
    while not self.gameOver:
        ...
        if self.catchExceptions:
          try:
            ...
            try:
              ...
              # TODO Why is this not a violation that is processed like the others?
      ...
      if self.catchExceptions:
        try:
          ...
          if crashAgentSoon:
            print "Exceeded the maximum number of warnings: %d" %self.totalAgentTimeWarnings[agentIndex]
            self._agentCrash(agentIndex, quiet=True)
